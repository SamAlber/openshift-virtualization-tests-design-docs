...
- [x] **Cross Integrations** — Does the feature affect other features or require testing by other teams? Identify the impact we cause.
  - *Details:* AWD interacts with hotplug (sig-virt); tests cover migration triggered by hotplug. Node drain is excluded (see Out of Scope). Cluster-level AWD configuration is validated as a dedicated test scenario (CNV-16551) within the AWD test suite.
...
- **[CNV-16551]** — As an admin, I want to enable AWD at the cluster level and have it apply to all qualifying migrations.
  - *Test Scenario:* [Tier 2, Gating] Enable AWD in the cluster-level migration configuration; migrate a VM that has no explicit AWD migration policy and verify the migration completes in Paused mode when pre-copy cannot converge — confirming the cluster-level setting is inherited.
  - *Priority:* P0
